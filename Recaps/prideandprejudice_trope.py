...
class TropeGraph():
    def __init__(self):
        self.G = nx.Graph()
        self.pandptropes = ["GoneHorriblyRight", "GreenEyedMonster", "SmugSnake", "AbhorrentAdmirer", "BitchInSheepsClothing",
                            "JerkWithAHeartOfGold", "IWantMyBelovedToBeHappy", "HappilyEverAfter"]
        ...
        self.supercat = "prideandprejudicemild"
    ...
    def add_trope_nodes(self):
        ...
        tropelist = []
        i = 0
        for trope in self.pandptropes:
            self.G.add_node(trope,label=trope)
            if trope in self.scoredict_mild:
                if 'normed mean happiness' in self.scoredict_mild[trope]:
                    whateverlist+=[self.scoredict_mild[trope]['normed mean happiness']]
                    tropelist += [trope]
                    i += 1
                else:
                    print("missing ",trope)
